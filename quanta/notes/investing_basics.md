...
- **Options**:  type of derivative that gives the holder the right, but not the obligation, to buy (call) or sell (put) an asset at a specific price before or on a specific date; includes longs and shorts
...
- **Forwards**: like futures, but are only traded over-the-counter (OTC) and not on an exchange; upon creation of contract, buyer and seller may customize the terms, size, and settlement process
- **Commodities**: basic physical goods that are interchangeable with other goods of the same type; often used as inputs in the production of other goods or services (hard: natural resources that are mined/extracted vs soft: agricultural products or livestock that are grown/raised)
- **Index Funds**: passive mutual funds or ETFs designed to track the performance of a specific market index; they offer broad market exposure and low fees - includes mutual funds and ETFs
...
- **Exchange Traded Funds (ETFs)**: like mutual funds, but trade on stock exchanges like individual stocks; ETFs typically track indices and are known for low costs and intraday liquidity
...
point i.e. 0.01%
- **Capacity**: how much a strategy is able to absorb without negatively impacting returns
